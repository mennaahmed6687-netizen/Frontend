    ...
    Promise.all([
        ...
        .then(function (results) {
            ...
            if (clubsMenuList) {
                clubsMenuList.innerHTML = clubs.length
                    ? clubs.map(function (club) {
                        ...
                        const targetUrl = 'clubs.html?clubId=' + club.id +
                            '&name=' + encodeURIComponent(club.name || '');
            ...
            if (!categories.length) {
                if (container) {
                    container.innerHTML =
                        '<p style="text-align:center;color:#64748B;">No activity categories found.</p>';
                }
                return;
            }
            ...
            if (menuList) {
                menuList.innerHTML = categories.map(function (category) {
                    ...
                    const targetUrl = 'activity-category.html?categoryId=' + category.id +
            ...
            container.innerHTML = categories.map(function (category) {
                ...
                // 🔥 FIX: استخدام صورة الـ category مباشرة (بدون activities)
                ...
                let desc = 'Explore ' + (category.name || 'activities') + ' activities.';
                ...
                return `
                    <div class="card" style="cursor:pointer;"
                        ...
                        <button onclick="event.stopPropagation(); window.location.href='${targetUrl}'">
                            Explore
                        </button>
                    </div>
                `;
